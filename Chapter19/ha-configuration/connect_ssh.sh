...
["backend0"]=`/usr/local/bin/terraform.py| jq '.backend.hosts[0]'` \
["backend1"]=`/usr/local/bin/terraform.py| jq '.backend.hosts[1]'` \
...
function connect_to_host  {
    ...
    then
        ...
        then
            ...
            new_window "ssh  -i /tmp/privkey.pem ubuntu@$ssh_host -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i /tmp/privkey.pem   -W %h:%p -q ubuntu@${cluster_hosts['bastion1']}\" -t \"echo \"PS1=$\"$host_code:$ssh_host$\"\">>~/.bashrc;bash\" 2>/dev/null "
