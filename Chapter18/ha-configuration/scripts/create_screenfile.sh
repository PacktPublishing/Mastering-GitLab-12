...
echo "screen ssh -i /tmp/mykey.pem ubuntu@$web0 -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -o StrictHostKeyChecking=no -i /tmp/mykey.pem   -W %h:%p -q ubuntu@$bastion0\" -t \"echo \"PS1=web1\"$\"\">>~/.bashrc;bash\"" >>superscreen
echo "screen ssh -i /tmp/mykey.pem ubuntu@$web1 -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -o StrictHostKeyChecking=no -i /tmp/mykey.pem   -W %h:%p -q ubuntu@$bastion1\" -t \"echo \"PS1=web2\"$\"\">>~/.bashrc;bash\"" >>superscreen
